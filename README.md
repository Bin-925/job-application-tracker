...
```mermaid
flowchart LR
    U["👤 사용자"] --> R["React SPA (Vite)"]
    R -->|"HTTP / JSON · JWT"| A["Spring Boot REST API"]
    A --> DB[("H2 Database")]
```

- React SPA ↔ Spring Boot REST API가 HTTP(JSON)로 통신
...
| 4️⃣ | **캘린더** | 지원일·면접일·마감일을 월별 캘린더에 색상 점으로 표시 · 날짜별 일정 조회 |
| 5️⃣ | **통계 대시보드** | 전체·진행 중·면접·최종합격 개수 집계 |
| 6️⃣ | **계정 관리** | 닉네임 수정 · 프로필 아바타(색상/이모지) 선택 · 회원 탈퇴 |
| 7️⃣ | **다크 모드** | 라이트/다크 테마 토글 |

> 🚧 **개발 예정** — 사람인 채용정보 API 연동(공고 검색 후 원클릭 등록) · 면접 D-day 알림
...
> JWT 시크릿은 `application-secret.yml`(gitignore 처리)에 설정합니다.
